...
func (s *DiscoveryServer) processRequest(req *discovery.DiscoveryRequest, con *Connection) error {
	...
	log.Debugf("ADS:%s: REQ %s resources type: %v, resources:%d nonce:%s version:%s ", stype,
		con.conID, req.TypeUrl, len(req.ResourceNames), req.ResponseNonce, req.VersionInfo)
	if req.TypeUrl == v3.HealthInfoType {
